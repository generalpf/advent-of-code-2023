...
int main()
{
    ...
    long mostEnergized = 0;
    std::vector<Laser> startingLasers;
    for (size_t y = 0; y < grid.size(); y++)
    {
        startingLasers.push_back(Laser { .x = -1L, .y = (long) y, .d = east });
        startingLasers.push_back(Laser { .x = (long) grid.front().length(), .y = (long) y, .d = west });
    }
    for (size_t x = 0; x < grid.front().length(); x++)
    {
        startingLasers.push_back(Laser { .x = (long) x, .y = -1L, .d = south });
        startingLasers.push_back(Laser { .x = (long) x, .y = (long) grid.size(), .d = north });
    }

    for (std::vector<Laser>::iterator startingLaser = startingLasers.begin(); startingLaser != startingLasers.cend(); startingLaser++)
    {
        std::vector<Laser> lasers;
        lasers.push_back(*startingLaser);
        ...
        if (visits > mostEnergized)
            mostEnergized = visits;
    }
    std::cout << "most energized = " << mostEnergized << std::endl;
}
